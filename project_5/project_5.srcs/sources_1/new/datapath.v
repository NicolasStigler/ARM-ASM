...
module datapath (
    ...
    input CBNZ_E,
    input SpecialInstrE,
    input LoadMultipleE,       // New signal for LDM instructions
    input StoreMultipleE,      // New signal for STM instructions
    input PreDecrementE,       // New addressing mode
    input PostIncrementE,      // New addressing mode
    output [31:0] PCF,
    input [31:0] InstrF,
    output [31:0] InstrD,
    ...
    wire [31:0] SpecialResultE;
    wire [31:0] SelectedResultE;
    ...
    // Signals for multiple load/store instructions
    reg [31:0] MemAddressE; // Current memory address for load/store
    reg [3:0] RegIndex;     // Index for registers to be loaded/stored
    reg [31:0] NextMemAddressE;
    ...
    // Handle Multiple Load/Store Instructions
    always @(*) begin
        if (StoreMultipleE || LoadMultipleE) begin
            // Pre-decrement or post-increment address calculation
            if (PreDecrementE)
                NextMemAddressE = MemAddressE - 4;
            else if (PostIncrementE)
                NextMemAddressE = MemAddressE + 4;
            else
                NextMemAddressE = MemAddressE; // No change

            // Update memory address
            MemAddressE = NextMemAddressE;
        end
    end
